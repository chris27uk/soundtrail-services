...
namespace Soundtrail.Services.Internal.Projector.Features.OnDiscoveryRequested.Adapters;

public sealed class DiscoveryRequestedSubscriptionHostedService(
    IDocumentStore documentStore,
    IServiceScopeFactory scopeFactory,
    ITypeRegistry registry,
    ILogger<DiscoveryRequestedSubscriptionHostedService> logger) : BackgroundService
{
    private const string SubscriptionName = "discovery-requested-catalog-search-dispatch";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(ex, "Discovery requested subscription failed.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        var handler = scope.ServiceProvider.GetRequiredService<OnDiscoveryRequestedHandler>();
                     ...
                     .GroupBy(item => item.StreamId, StringComparer.Ordinal))
        {
            var events = stream
                ...
                .Where(item => item.Event is DiscoveryRequested)
                .ToArray();
            ...
            await handler.Handle(
                new DiscoveryRequestedCommand(
                    DiscoveryQueryKey.For(DiscoveryQueryKey.ToMusicSearchCriteria(stream.Key)),
                    events),
                cancellationToken);
        }
    }
