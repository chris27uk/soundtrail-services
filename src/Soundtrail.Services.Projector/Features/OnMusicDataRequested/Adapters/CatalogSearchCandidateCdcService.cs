...
internal sealed class CatalogSearchCandidateCdcService(
    ...
    IDocumentStore documentStore) : BackgroundService
{
    ...
    private async Task CatchUpAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var handler = scope.ServiceProvider.GetRequiredService<CatalogSearchCandidateProjectorHandler>();
            ...
            foreach (var storedEvent in events)
            {
                switch (storedEvent.EventType)
                {
                    ...
                    default:
                        throw new InvalidOperationException($"Unsupported catalog search candidate event type '{storedEvent.EventType}'.");
                }
