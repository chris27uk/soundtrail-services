...
{
    private const string CheckpointId = "projector/checkpoints/work-requested";
    ...
    private async Task CatchUpAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var handler = scope.ServiceProvider.GetRequiredService<WorkRequestedProjectorHandler>();
            ...
            var query = session.Query<RavenStoredEventRecord>()
                .Where(x => x.AggregateType == "catalog-search" && x.EventType == "work-requested");
