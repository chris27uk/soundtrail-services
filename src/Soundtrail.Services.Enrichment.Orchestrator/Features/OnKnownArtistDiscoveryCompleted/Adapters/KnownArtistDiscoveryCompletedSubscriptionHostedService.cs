...
namespace Soundtrail.Services.Enrichment.Orchestrator.Features.OnKnownArtistDiscoveryCompleted.Adapters;

public sealed class KnownArtistDiscoveryCompletedSubscriptionHostedService(
    IDocumentStore documentStore,
    IServiceScopeFactory scopeFactory,
    ITypeRegistry registry,
    ILogger<KnownArtistDiscoveryCompletedSubscriptionHostedService> logger) : BackgroundService
{
    private const string SubscriptionName = "known-artist-discovery-completed-artist-catalog";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(ex, "Known artist discovery completed subscription failed.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        var handler = scope.ServiceProvider.GetRequiredService<ApplyKnownArtistDiscoveryCompletedToArtistCatalogHandler>();
                     ...
                     .GroupBy(item => item.StreamId, StringComparer.Ordinal))
        {
            var events = stream
                ...
                .Where(item => item.Event is KnownArtistDiscoveryCompleted)
                .ToArray();
            ...
            await handler.Handle(
                new KnownArtistDiscoveryCompletedCommand(stream.Key, events),
                cancellationToken);
        }
    }
}
