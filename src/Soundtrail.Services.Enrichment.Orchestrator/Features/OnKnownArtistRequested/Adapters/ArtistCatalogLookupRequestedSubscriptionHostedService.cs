...
public sealed class ArtistCatalogLookupRequestedSubscriptionHostedService(
    IDocumentStore documentStore,
    IServiceScopeFactory scopeFactory,
    ITypeRegistry registry,
    ILogger<ArtistCatalogLookupRequestedSubscriptionHostedService> logger) : BackgroundService
{
    private const string SubscriptionName = "artist-catalog-lookup-requested-dispatch";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(ex, "Artist catalog lookup requested subscription failed.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        var handler = scope.ServiceProvider.GetRequiredService<DispatchKnownArtistLookupCommandHandler>();
                     ...
                     .GroupBy(item => item.StreamId, StringComparer.Ordinal))
        {
            var events = stream
                ...
                .Where(item => item.Event is ArtistCatalogLookupRequested)
                .ToArray();
            ...
            await handler.Handle(
                new ArtistCatalogLookupRequestedCommand(stream.Key, events),
                cancellationToken);
        }
    }
}
