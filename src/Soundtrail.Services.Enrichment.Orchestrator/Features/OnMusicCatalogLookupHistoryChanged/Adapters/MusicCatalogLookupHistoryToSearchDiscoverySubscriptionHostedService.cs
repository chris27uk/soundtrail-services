...
public sealed class MusicCatalogLookupHistoryToSearchDiscoverySubscriptionHostedService(
    IDocumentStore documentStore,
    IServiceScopeFactory scopeFactory,
    ITypeRegistry registry,
    ILogger<MusicCatalogLookupHistoryToSearchDiscoverySubscriptionHostedService> logger) : BackgroundService
{
    private const string SubscriptionName = "music-catalog-lookup-history-to-search-discovery";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(ex, "Music catalog lookup history to search discovery subscription failed.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        var handler = scope.ServiceProvider.GetRequiredService<ApplyMusicCatalogLookupHistoryChangedToSearchDiscoveryHandler>();
