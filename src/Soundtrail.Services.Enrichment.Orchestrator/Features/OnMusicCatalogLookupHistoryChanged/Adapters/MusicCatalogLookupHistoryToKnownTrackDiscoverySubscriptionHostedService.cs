...
public sealed class MusicCatalogLookupHistoryToKnownTrackDiscoverySubscriptionHostedService(
    IDocumentStore documentStore,
    IServiceScopeFactory scopeFactory,
    ITypeRegistry registry,
    ILogger<MusicCatalogLookupHistoryToKnownTrackDiscoverySubscriptionHostedService> logger) : BackgroundService
{
    private const string SubscriptionName = "music-catalog-lookup-history-to-known-track-discovery";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(ex, "Music catalog lookup history to known-track discovery subscription failed.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        var handler = scope.ServiceProvider.GetRequiredService<ApplyMusicCatalogLookupHistoryChangedToKnownTrackDiscoveryHandler>();
