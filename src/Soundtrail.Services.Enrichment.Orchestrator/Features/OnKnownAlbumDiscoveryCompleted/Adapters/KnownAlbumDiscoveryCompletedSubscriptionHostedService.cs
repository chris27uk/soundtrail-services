...
namespace Soundtrail.Services.Enrichment.Orchestrator.Features.OnKnownAlbumDiscoveryCompleted.Adapters;

public sealed class KnownAlbumDiscoveryCompletedSubscriptionHostedService(
    IDocumentStore documentStore,
    IServiceScopeFactory scopeFactory,
    ITypeRegistry registry,
    ILogger<KnownAlbumDiscoveryCompletedSubscriptionHostedService> logger) : BackgroundService
{
    private const string SubscriptionName = "known-album-discovery-completed-artist-catalog";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(ex, "Known album discovery completed subscription failed.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        var handler = scope.ServiceProvider.GetRequiredService<ApplyKnownAlbumDiscoveryCompletedToArtistCatalogHandler>();
                     ...
                     .GroupBy(item => item.StreamId, StringComparer.Ordinal))
        {
            var events = stream
                ...
                .Where(item => item.Event is KnownAlbumDiscoveryCompleted)
                .ToArray();
            ...
            await handler.Handle(
                new KnownAlbumDiscoveryCompletedCommand(stream.Key, events),
                cancellationToken);
        }
    }
}
