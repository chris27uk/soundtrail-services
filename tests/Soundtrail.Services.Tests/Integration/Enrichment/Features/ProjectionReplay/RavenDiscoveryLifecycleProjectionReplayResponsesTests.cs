...
public sealed class RavenDiscoveryLifecycleProjectionReplayResponsesTests
{
    ...
    public async Task Given_Discovery_Lifecycle_Events_When_Replayed_Then_The_Status_Document_Is_Projected()
    {
        ...
        var loaded = await SearchDiscoveryHistory.LoadAsync(repository, criteria, CancellationToken.None);
        ...
        loaded = await SearchDiscoveryHistory.LoadAsync(repository, criteria, CancellationToken.None);
        ...
        var criteriaPersistentId = DiscoveryQueryKey.StableValueFor(criteria);
