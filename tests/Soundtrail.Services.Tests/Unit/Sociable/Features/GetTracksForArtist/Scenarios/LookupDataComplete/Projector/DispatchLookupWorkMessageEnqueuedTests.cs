...
namespace Soundtrail.Services.Tests.Unit.Sociable.Features.GetTracksForArtist.Scenarios.LookupDataComplete.Projector;
...
public sealed class DispatchLookupWorkMessageEnqueuedTests
{
    ...
    public async Task Then_The_Correlation_Id_Is_Preserved()
    {
        ...
        Message(environment).CorrelationId.Value
            .Should().Be($"work-scheduled:child_tracks_for_artist:{environment.ArtistId.Value}:{requestTime:O}");
    }
    ...
    private static DispatchLookupWork Message(GetTracksForArtistSociableTestEnvironment environment) =>
        environment.SentMessages<DispatchLookupWork>()
            .Single(message => message.Target.NormalisedIdentifier == $"child_tracks_for_artist:{environment.ArtistId.Value}");
