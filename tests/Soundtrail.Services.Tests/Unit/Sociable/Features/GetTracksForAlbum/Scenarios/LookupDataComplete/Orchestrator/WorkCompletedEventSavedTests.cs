...
public sealed class WorkCompletedEventSavedTests
{
    ...
    private static WorkCompleted Event(GetTracksForAlbumSociableTestEnvironment environment) =>
        environment.SavedEvents<WorkCompleted>()
            .First(@event => @event.Target.NormalisedIdentifier == $"child_tracks_for_album:{environment.AlbumId.StableValue}");
