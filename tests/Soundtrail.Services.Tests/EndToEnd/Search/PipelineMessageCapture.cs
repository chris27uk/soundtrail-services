...
public sealed class PipelineMessageCapture
{
    ...
    private readonly ConcurrentDictionary<Type, ConcurrentQueue<object>> allReceived = new();

    public void Record<TMessage>(TMessage message) where TMessage : class
    {
        allReceived.GetOrAdd(typeof(TMessage), _ => new ConcurrentQueue<object>())
            .Enqueue(message);
    ...
    public int Count<TMessage>() where TMessage : class =>
        allReceived.TryGetValue(typeof(TMessage), out var messages)
            ? messages.Count
            : 0;
}
